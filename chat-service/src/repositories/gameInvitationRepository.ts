...
export function createInvitation(from_user_id: number, to_user_id: number, game_type: string = 'pong', expiresInMinutes: number = 2) {
    const expires_at = new Date(Date.now() + expiresInMinutes * 60 * 1000).toISOString();
    const stmt = db.prepare("INSERT INTO game_invitations (from_user_id, to_user_id, game_type, expires_at) VALUES (?, ?, ?, ?)");
    const result = stmt.run(from_user_id, to_user_id, game_type, expires_at);
